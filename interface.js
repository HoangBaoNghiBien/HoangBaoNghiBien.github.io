...
/////////////////////////////////////////////////////////////////////////////
// Keyboard interactions (number keys, arrow keys)
/////////////////////////////////////////////////////////////////////////////

// Set select on sudoku cell and use arrow keys to move focus.
$(document).on('keydown', function(ev) {
  // Handle arrow keys
  if (ev.keyCode >= 37 && ev.keyCode <= 40) {
    var pos = 0;
    if (visiblefocus !== null) {
      pos = parseInt($(visiblefocus).attr('id').substr(2));
    }
    var newpos = pos;
    if (ev.keyCode == 37) { newpos -= 1; }
    if (ev.keyCode == 38) { newpos -= Sudoku.N; }
    if (ev.keyCode == 39) { newpos += 1; }
    if (ev.keyCode == 40) { newpos += Sudoku.N; }
    if (newpos >= 0 && newpos < Sudoku.S) {
      setvisiblefocus($('#sc' + newpos));
      // Log the focus change event
      $(document).trigger('log', ['focusChange', { fromCell: pos, toCell: newpos, timestamp: new Date().toISOString() }]);
    }
    ev.preventDefault();
    ev.stopPropagation();
    return;
  ...
  if (ev.keyCode >= 49 && ev.keyCode <= 52) {
    var num = parseInt(ev.keyCode) - 48;
    setcurnumber(num);
    applyNumberToCell(num);

    // Log the number key press event
    var pos = 0;
    if (visiblefocus !== null) {
      pos = parseInt($(visiblefocus).attr('id').substr(2));
    }
    $(document).trigger('log', ['numberKeyPress', { cell: pos, number: num, timestamp: new Date().toISOString() }]);
  }
})

// logic to set the number to the cell
...
$(document).on('mousedown', 'td.sudoku-cell', function(ev) {
  ev.preventDefault();
  hidepopups();
  // listen for key board press on number keys then get the number
...
// If user press >, go to the next game.
...
// If user press <, go to the previous game.
$(document).on('keydown', function (event) {
...
// If user press c, clear the answers.  
$(document).on('keydown', function (event) {
  if (event.keyCode == 67) {
    hidepopups();
    var state = currentstate();
...
// If user press Enter, check the answers.
$(document).on('keydown', function (event) {
  if (event.keyCode == 13) {
    hidepopups();
    var state = currentstate();
...
// Tooltips
/////////////////////////////////////////////////////////////////////////////
// If user hover the clear button, show the tooltip class="tooltiptext" "Press 'c'"
$(document).ready(function () {
  // Log mouse interactions
  function logMouseInteraction(buttonID, actionType) {
    $(document).trigger('log', ['mouseInteraction', { button: buttonID, action: actionType, timestamp: new Date().toISOString() }]);
  }

  // Clear
  // ------------------------------------------------------
  // Set the tooltip text when mouse enters the button
  $('#clearbutton').on('mouseenter', function () {
    $(this).next('.tooltiptext').text("Press 'c'");
    logMouseInteraction('clearbutton', 'mouseenter');
  });

  // Clear the tooltip text when mouse leaves
  $('#clearbutton').on('mouseleave', function () {
    $(this).next('.tooltiptext').text("");
    logMouseInteraction('clearbutton', 'mouseleave');
  });

  // Check
  // ------------------------------------------------------
  // Set the tooltip text when mouse enters the button
  $('#checkbutton').on('mouseenter', function () {
    $(this).next('.tooltiptext').text("Press Enter");
    logMouseInteraction('checkbutton', 'mouseenter');
  });

  // Clear the tooltip text when mouse leaves
  $('#checkbutton').on('mouseleave', function () {
    $(this).next('.tooltiptext').text("");
    logMouseInteraction('checkbutton', 'mouseleave');
  });
  ...
  $('#prevbutton').on('mouseenter', function () {
    $(this).next('.tooltiptext').text("Press '<'");
    logMouseInteraction('prevbutton', 'mouseenter');
  });

  // Clear the tooltip text when mouse leaves
  $('#prevbutton').on('mouseleave', function () {
    $(this).next('.tooltiptext').text("");
    logMouseInteraction('prevbutton', 'mouseleave');
  });
  ...
  $('#nextbutton').on('mouseenter', function () {
    $(this).next('.tooltiptext').text("Press '>'");
    logMouseInteraction('nextbutton', 'mouseenter');
  }
  );
  ...
  $('#nextbutton').on('mouseleave', function () {
    $(this).next('.tooltiptext').text("");
    logMouseInteraction('nextbutton', 'mouseleave');
  });

  // // Eraser
  // // ------------------------------------------------------
  // // Set the tooltip text when mouse enters the button
  // $('#eraserbutton').on('mouseenter', function () {
  //   $(this).next('.tooltiptext').text("Press 'e'");
  // }
  // );

  // // Clear the tooltip text when mouse leaves
  // $('#eraserbutton').on('mouseleave', function () {
  //   $(this).next('.tooltiptext').text("");
  // });
...
// If user press e, erase the number on the selected cell.
$(document).on('keydown', function (event) {
  if (event.keyCode == 69) {
    var pos = 0;
    if (visiblefocus !== null) {
...
// Hide the popup after 1000ms after user press enter
$(document).on('keydown', function (event) {
  // 13
  if (event.keyCode == 13) {
    setTimeout(function () {
...
function numberkeyhtml() {
  ...
  for (var j = 1; j <= Sudoku.N; ++j) {
    result += '<tr><td class="numberkey-cell" id="nk' + j + '">' +
      '<div class="sudoku-answer" tabindex="0" nk="' + j + '">' +
      handglyph(j) + '</div></td></tr>';
  }
  result +=
    '<tr>' +
    '<td class="numberkey-cell" id="nk0">' +
    '<div class="tooltip2">' +
    '<div class="eraser nk0">&#xf12d;</div>' +
    '<span class="tooltiptext2">Press \'e\'</span>' +
    '</div>' +
    '</td>' +
    '</tr>';
  result += '</table>';
